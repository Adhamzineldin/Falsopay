...
class InstantPaymentAddressesRoute {
    public static function define($router, array $middlewares = []) {
        ...
        $router->add('GET', '/api/ipa/{bank_id}/{account_id}', [InstantPaymentAddressController::class, 'getByBankAndAccount']);
        $router->add('PUT', '/api/ipa/{bank_id}/{account_id}', [InstantPaymentAddressController::class, 'updateInstantPaymentAddress']);
        $router->add('DELETE', '/api/ipa/{bank_id}/{account_id}', [InstantPaymentAddressController::class, 'deleteInstantPaymentAddress']);
    }
}
