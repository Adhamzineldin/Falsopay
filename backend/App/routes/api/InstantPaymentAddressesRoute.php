...
class InstantPaymentAddressesRoute {
    public static function define($router, array $middlewares = []) {
        ...
        $router->add('PUT', '/api/ipa/update-pin', [InstantPaymentAddressController::class, 'updatePinForIpa']);
        $router->add('POST', '/api/ipa/verify-pin', [InstantPaymentAddressController::class, 'verifyPinForIpa']);
    }
}
